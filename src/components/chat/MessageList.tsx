                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation": {
                            ...
                            let Icon = FilePen;
                            let label = "Editing file";
                            if (tool.toolName === "str_replace_editor") {
                              if (args?.command === "create") { Icon = FilePlus; label = `Creating ${fileName ?? "file"}`; }
                              else if (args?.command === "view") { Icon = FileSearch; label = `Reading ${fileName ?? "file"}`; }
                              else { Icon = FilePen; label = `Editing ${fileName ?? "file"}`; }
                            } else if (tool.toolName === "file_manager") {
                              if (args?.command === "delete") { Icon = Trash2; label = `Deleting ${fileName ?? "file"}`; }
                              else { Icon = FolderInput; label = `Moving ${fileName ?? "file"}${newFileName ? ` → ${newFileName}` : ""}`; }
                            }
